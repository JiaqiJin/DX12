...
namespace RHI
{
	// Represents a variable in the Shader, external resources can be bound through this variable (a constant or a map)
	...
	public:
		...
		/* Only CBV and other Buffer SRV/UAV can be bound as Root Descriptor, because when 
		* calling ID3D12GraphicsCommandList::SetGraphicsRootConstantBufferView() function,
		* The size of Buffer is not specified. The size of Buffer is determined by Shader. 
		* Because Texture needs a lot of information to describe, it is not enough to provide only one address.
	...
	*/
	...
	private:
		...
		// Because ShaderVariable cannot be copied and cannot be moved, its pointer is saved
