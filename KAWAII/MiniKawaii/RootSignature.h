...
namespace MyDirectX
{
	...
	/* Maximum 64 DWORDS divied up amongst all root parameters root constants = 1 DWORD * numConstants
	root descriptor (CBV, SRV, or UAV) = 2 DWORDS each descriptor table pointer = 1 DWORD
	...
	*/
	...
	public:
		...
		void Reset(UINT numRootRarams, UINT numStaticSamplers)
		{
			if (numRootRarams > 0)
		...
		void InitStaticSampler(UINT reg, const D3D12_SAMPLER_DESC& nonStaticSamplerDesc, D3D12_SHADER_VISIBILITY visibility);
		void Finalize(ID3D12Device* pDevice, const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS flags);
