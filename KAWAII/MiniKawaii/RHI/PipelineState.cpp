...
namespace RHI
{
		...
		m_RootSignature{ renderDevice }
	{
		...
		if (m_Desc.PipelineType == PIPELINE_TYPE_GRAPHIC)
		{
			...
			assert(m_Shaders.size() > 0 && "No Shader");
		...
		// COMPUTE PIPELINE AND MESH PIPELINE
	}
	...
	{
		const static SHADER_RESOURCE_VARIABLE_TYPE mutableDynamicVarType[] = 
