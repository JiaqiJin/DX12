...
namespace RHI
{
	...
	{	// Texture2D g_Tex, and Texture2D g_Tex[4],the name will be deduced as"g_Tex". cbuffer cbBuff0 : register(b5), the name "cbBuffer0"
		...
		static constexpr const UINT32 ShaderInputTypeBits = 4; // Identifes resource type adn takes values from D3D_SHADER_INPUT_TYPE enumeration.
	...
	private:
		...
		// Verify that the Bits occupied by each attribute are sufficient
	...
	{
	public:
		// Initialize the Shader resource from the compiled bytecode
...
Exampler : 
	...
	Texture2D g_Tex2[8]  : register(t4);
	SamplerState g_Tex0_sampler    : register(s3);
	SamplerState g_Tex2_sampler[8] : register(s5);
...

Representations : 
 |                    |               |                      | ......
 |  Name = "cbBuff0"  |  Name = "cbBuff1"  | Name = "g_Tex2" | ......
 |  BindPoint = 5	  |  BindPoint = 2     |  BindPoint = 4  | ......
 |  BindCount = 1     |  BindCount = 1     |  BindCount = 8  | ......
*/
